...
public struct DocumentBlock: Codable, Sendable {
    ...
    public init(name: String, format: Format, source: Source) throws {
        ...
        guard !name.isEmpty else {
            throw BedrockLibraryError.invalidName("Document name is not allowed to be empty")
        }
        ...
        guard name.count <= 200 else {
            throw BedrockLibraryError.invalidName("Document name must be no longer than 200 characters")
        }
    ...
    public enum Source: Codable, Sendable {
        ...
        public init(bytes: String) throws {
            guard !bytes.isEmpty else {
                throw BedrockLibraryError.invalidName("Document source is not allowed to be empty")
            }
            self = .bytes(bytes)
        }
