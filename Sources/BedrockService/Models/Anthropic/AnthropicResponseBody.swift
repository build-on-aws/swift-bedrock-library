...
public struct AnthropicResponseBody: ContainsTextCompletion {
    ...
    public func getTextCompletion() throws -> TextCompletion {
        guard content.count > 0 else {
            throw BedrockLibraryError.completionNotFound("AnthropicResponseBody: content is empty")
        }
        guard let completion = content[0].text else {
            throw BedrockLibraryError.completionNotFound("AnthropicResponseBody: content[0].text is nil")
        }
